...
void Sender::response(Message m, int step) {
	if (step == 2) {
		if (m.IDA == this->id && m.IDB == this->recvClient->id)
		{
			...
			this->symmetricKeyFuncs.decrypt(m.sessionKey, this->sessionKey);
			this->symmetricKeyFuncs.decrypt(m.IDB, this->recvClient->id);
		}
		...
		Message message;
		this->symmetricKeyFuncs.decrypt(m.nonce, message.nonce);
		message.nonce[0]+=1;// = 1;
		this->symmetricKeyFuncs.encrypt(message.nonce, message.nonce);

		message.info = "This is message encrypted by A";
		this->symmetricKeyFuncs.encrypt(message.info, message.info);
		...
		this->symmetricKeyFuncs.setKey(this->sessionKey);
		bool result = this->symmetricKeyFuncs.decrypt(m.info, m.info);
		if (result == true) {
...
void Receiver::response(Message m, int step) {
	...
	if (step == 3) {
		...
		this->symmetricKeyFuncs.decrypt(m.sessionKey, this->sessionKey);
		string tmp;
		this->symmetricKeyFuncs.decrypt(m.IDA, tmp);
		...
		string tmp;
		this->symmetricKeyFuncs.decrypt(m.nonce, tmp);
...
void KDC::response(Message m, int step, Message*& res) {
	if (step == 1) {
		...
		this->symmetricKeyFuncs.encrypt(Ks, res->sessionKey);
		this->symmetricKeyFuncs.encrypt(m.IDA, res->IDA);
		this->symmetricKeyFuncs.encrypt(m.IDB, res->IDB);
		this->symmetricKeyFuncs.encrypt(m.nonce, res->nonce);
		
		res->msg = new Message;

		//The message includes two items intended for B encrypted by the master key of B
		this->symmetricKeyFuncs.setKey(masterKeyB);
		this->symmetricKeyFuncs.encrypt(Ks, res->msg->sessionKey);
		this->symmetricKeyFuncs.encrypt(m.IDA, res->msg->IDA);
