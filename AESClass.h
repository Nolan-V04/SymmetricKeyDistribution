...
public:
	...
	void setKey(string newKey) {
		...
		decoder.Put((CryptoPP::byte*)newKey.data(), newKey.size());
		decoder.MessageEnd();
		this->key = SecByteBlock((const CryptoPP::byte*)decodedKey.data(), decodedKey.size());
	}

	bool encrypt(string input, string &result) {
		string ciphered;
		try {
	...
	bool decrypt(string input, string &result) {
		string deciphered;
		try {
